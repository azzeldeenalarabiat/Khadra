...
- **No FK cascades in the database. Ever.** Every constraint is restricting — `ON DELETE RESTRICT` or `ON DELETE NO ACTION`, which both refuse — and never `CASCADE` or `SET NULL`. In the MODEL that is `DeleteBehavior.Restrict` for a reference to a lookup or to another aggregate, and `DeleteBehavior.ClientCascade` for a child collection **the aggregate itself removes from**, where EF deletes the orphan rather than throwing "the association between entity types … has been severed". (`ClientCascade` emits `NO ACTION` instead of `RESTRICT`; in PostgreSQL the only difference is deferrability, and neither one cascades.) `Restrict` on a removable child is not the stricter choice, it is the broken one: the child's FK is not nullable, so severing it throws at `SaveChanges` and the endpoint answers 500. It did — for removing a vehicle photo and for un-saving a car — until 2026-09-21. `RemovingAChildFromAnAggregateTests` pins both halves: no cascading constraint anywhere in the relational model, and `ClientCascade` on the two removable collections. Smart enums persisted via `HasConversion` to their string name. Value objects via `OwnsOne` or converters.
