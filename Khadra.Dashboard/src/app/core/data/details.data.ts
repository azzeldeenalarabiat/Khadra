...
export const APPLICATION_TIMELINE: readonly TimelineStep[] = [
  ...
  { label: 'Approved / Rejected', meta: 'Due 03 Sep 2026 08:14 — 7h 12m left', tone: 'warn', future: true },
];
...
export const DEALER_TABS = ['Overview', 'Documents', 'Cars', 'Bookings', 'Employees', 'Financial', 'Reviews', 'Activity'] as const;
...
export const DEALER_VERIFICATION: readonly { label: string; meta: string; icon: IconName; tone: Tone }[] = [
...
export const DEALER_TAB_TABLES: Readonly<Record<string, TableConfig>> = {
  Cars: {
    ...
    rows: [
      ...
      cells: [
        ...
        { kind: 'text' as const, value: r[4], align: 'right' as const },
  ...
  Employees: {
    ...
    ].map((r) => ({ cells: r.map((v) => ({ kind: 'text' as const, value: v })) })),
  },
  Financial: {
    ...
    rows: [
      ...
      cells: [
        { kind: 'text' as const, value: r[0] },
        { kind: 'text' as const, value: r[1] },
        { kind: 'text' as const, value: r[2], align: 'right' as const },
        { kind: 'text' as const, value: r[3], align: 'right' as const },
        { kind: 'text' as const, value: r[4], align: 'right' as const },
        { kind: 'badge' as const, value: r[5], tone: r[6] as Tone },
      ],
    })),
  ...
  Reviews: {
    ...
    rows: [
      ['Layla Odeh', '4.8', 'Car was clean and delivery was on time in Amman.', 'BK-20902', 'Published', 'ok'],
...
export const BOOKING_DETAIL = {
  ...
  alert: 'Deposit payment failed — this booking auto-cancels in 4 hours unless the deposit is captured.',
};

export const BOOKING_CARDS: readonly { title: string; icon: IconName; rows: readonly KeyValue[] }[] = [
  {
...
export const BOOKING_TIMELINE: readonly TimelineStep[] = [
  ...
  { label: 'Deposit payment attempted', meta: '03 Sep 2026 09:14 · failed, card declined', tone: 'bad' },
...
export const CUSTOMER_TABS = ['Overview', 'Bookings', 'Reviews', 'Verification', 'Activity'] as const;
...
export const CUSTOMER_DOCUMENTS: readonly DocumentTile[] = [
  { label: 'National ID', status: 'Verified', tone: 'ok', file: 'national-id.jpg · restricted', meta: 'Expires 12 Aug 2029' },
  { label: 'Driving licence', status: 'Verified', tone: 'ok', file: 'licence.jpg · restricted', meta: 'Expires 03 Mar 2027' },
  { label: 'Passport', status: 'Not required', tone: 'dim', file: 'not uploaded', meta: 'Required for foreign customers only' },
];
...
export const CUSTOMER_TAB_TABLES: Readonly<Record<string, TableConfig>> = {
  ...
  Reviews: {
    ...
    rows: [
      ['Al-Nadeem Rentals', '4.8', 'Car was clean and delivery was on time in Amman.', 'BK-20902', 'Published', 'ok'],
      ['Aqaba Coast Cars', '5.0', 'Very smooth pickup at the airport.', 'BK-20790', 'Published', 'ok'],
    ].map((r) => ({
      cells: [
        { kind: 'text' as const, value: r[0] },
        { kind: 'text' as const, value: `${r[1]} ★` },
...
export const PAYMENT_HISTORY: readonly TimelineStep[] = [
  ...
  { label: 'Booking hold expires', meta: '03 Sep 2026 13:14 · auto-cancel', tone: 'warn', future: true },
];
...
export const DISPUTE_CARDS: readonly { title: string; icon: IconName; rows: readonly KeyValue[] }[] = [
  {
    title: 'Booking',
    icon: 'receipt',
    rows: [
...
export const DISPUTE_EVIDENCE: readonly { label: string; by: string; tone: Tone; ts: string; file: string }[] = [
  { label: 'Front bumper at pickup', by: 'Dealer', tone: 'dim', ts: '27 Aug 09:04', file: 'pickup-front.jpg' },
  { label: 'Front bumper at return', by: 'Dealer', tone: 'dim', ts: '30 Aug 18:22', file: 'return-front.jpg' },
  { label: 'Customer photo at pickup', by: 'Customer', tone: 'accent', ts: '27 Aug 09:01', file: 'customer-pickup.jpg' },
  { label: 'Repair quote', by: 'Dealer', tone: 'dim', ts: '31 Aug 11:40', file: 'repair-quote.pdf' },
];

export const DISPUTE_TIMELINE: readonly TimelineStep[] = [
  { label: 'Dispute opened by customer', meta: '27 Aug 2026 08:41 · damage penalty disputed', tone: 'bad' },
...
export const COMMISSION_ROWS: readonly {
  id: string; dealer: string; value: string; rate: string; amount: string; payout: string; status: string; tone: Tone; date: string;
}[] = [
  { id: 'BK-20984', dealer: 'Al-Nadeem Rentals', value: '1,240', rate: '20%', amount: '248', payout: '992', status: 'Paid', tone: 'ok', date: '01 Sep' },
  { id: 'BK-20977', dealer: 'Aqaba Coast Cars', value: '860', rate: '20%', amount: '172', payout: '688', status: 'Paid', tone: 'ok', date: '01 Sep' },
  { id: 'BK-20961', dealer: 'Petra Wheels', value: '2,105', rate: '18%', amount: '379', payout: '1,726', status: 'Processing', tone: 'warn', date: '02 Sep' },
  { id: 'BK-20940', dealer: 'Wadi Rum Motors', value: '640', rate: '20%', amount: '128', payout: '512', status: 'Pending', tone: 'dim', date: '02 Sep' },
  { id: 'BK-20933', dealer: 'Irbid Auto Lease', value: '1,480', rate: '15%', amount: '222', payout: '1,258', status: 'Pending', tone: 'dim', date: '02 Sep' },
  { id: 'BK-20918', dealer: 'Dead Sea Drive', value: '990', rate: '20%', amount: '198', payout: '792', status: 'Failed', tone: 'bad', date: '01 Sep' },
  { id: 'BK-20902', dealer: 'Al-Nadeem Rentals', value: '3,320', rate: '20%', amount: '664', payout: '2,656', status: 'Paid', tone: 'ok', date: '01 Sep' },
  { id: 'BK-20896', dealer: 'Jerash Rentals', value: '710', rate: '20%', amount: '142', payout: '568', status: 'Paid', tone: 'ok', date: '01 Sep' },
];
...
export const SETTINGS_GROUPS: readonly { title: string; icon: IconName; note: string; rows: readonly SettingRow[] }[] = [
  {
    title: 'Booking rules',
    icon: 'calendar-check',
    note: 'Applied at booking creation',
    rows: [
      { label: 'Platform commission', desc: 'Share of each booking retained by the platform. Range 15–20%.', value: '20%', by: 'Rania Haddad', when: '02 Sep 2026 14:05' },
      { label: 'No-show timeout', desc: 'Hours after pickup time before a booking is marked no-show.', value: '8h', by: 'Rania Haddad', when: '18 Jul 2026 09:31' },
      { label: 'Cancellation grace period', desc: 'Free cancellation window after booking confirmation.', value: '24h', by: 'Omar Deeb', when: '02 May 2026 16:40' },
      { label: 'Default delivery fee', desc: 'Charged when a dealer delivers outside their free radius.', value: '8 JOD', by: 'Rania Haddad', when: '12 Mar 2026 11:02' },
    ],
  },
  {
    title: 'Penalty rules',
    ...
    rows: [
      { label: 'Dealer non-delivery penalty', desc: 'Charged to the dealer when a confirmed vehicle is not delivered.', value: '50 JOD', by: 'Rania Haddad', when: '21 Jun 2026 13:55' },
      { label: 'Customer no-show penalty', desc: 'Share of the deposit forfeited on a no-show.', value: '100%', by: 'Yousef Barakat', when: '21 Jun 2026 13:58' },
      { label: 'Late return charge', desc: 'Extra day charged after this grace period.', value: '2h', by: 'Omar Deeb', when: '09 Feb 2026 08:12' },
    ],
  },
  {
    ...
    rows: [
      { label: 'Minimum renter age', desc: 'Customers below this age cannot complete a booking.', value: '21', by: 'Rania Haddad', when: '04 Jan 2025 10:00' },
      { label: 'ID requirement', desc: 'National ID for residents, passport for foreign customers.', value: 'Required', by: 'Rania Haddad', when: '04 Jan 2025 10:00' },
      { label: 'Licence held for', desc: 'Minimum time since the driving licence was issued.', value: '2 years', by: 'Omar Deeb', when: '15 Apr 2026 12:20' },
      { label: 'Foreign customer deposit', desc: 'Additional deposit for customers without a Jordanian ID.', value: '150 JOD', by: 'Yousef Barakat', when: '15 Apr 2026 12:24' },
    ],
  },
];
...
export const NOTIFICATIONS: readonly {
  title: string; desc: string; ts: string; icon: IconName; tone: Tone; unread: boolean; link: string; route: string;
}[] = [
  { title: 'Dealer application approaching SLA', desc: 'Wadi Rum Motors · 7h 12m left of the 48-hour window', ts: '12 min ago', icon: 'storefront', tone: 'warn', unread: true, link: 'Review', route: '/dealers/review' },
  { title: 'New dispute opened', desc: 'DSP-1207 · vehicle not delivered · Al-Nadeem Rentals', ts: '38 min ago', icon: 'scales', tone: 'warn', unread: true, link: 'Open', route: '/disputes/detail' },
  { title: 'Payment failed', desc: 'PAY-99321 · JOD 120 deposit · card declined', ts: '1h ago', icon: 'credit-card', tone: 'bad', unread: true, link: 'Open', route: '/payments/detail' },
  { title: 'Dispute breached the 48-hour SLA', desc: 'DSP-1184 · 13h over · escalated to High', ts: '2h ago', icon: 'scales', tone: 'bad', unread: true, link: 'Resolve', route: '/disputes/detail' },
  { title: 'Payout batch pending approval', desc: 'PR-0091 · JOD 18,420 across 22 dealers', ts: '5h ago', icon: 'arrow-line-up-right', tone: 'dim', unread: true, link: 'Review', route: '/payouts' },
  { title: 'Suspicious sign-in blocked', desc: 'omar@carrental.jo · 4 failed attempts from Bucharest', ts: '7h ago', icon: 'shield-check', tone: 'warn', unread: true, link: 'Review', route: '/security' },
  { title: 'Dealer approved', desc: 'Aqaba Coast Cars · approved by you', ts: 'Yesterday', icon: 'check-circle', tone: 'ok', unread: false, link: 'View', route: '/dealers/profile' },
  { title: 'Payout run completed', desc: 'PR-0090 · JOD 21,300 paid to 19 dealers', ts: 'Yesterday', icon: 'arrow-line-up-right', tone: 'ok', unread: false, link: 'View', route: '/payouts' },
];

/* ── Security ──────────────────────────────────────────────────────────── */

export const SESSIONS: readonly { device: string; meta: string; icon: IconName; current: boolean; revocable: boolean }[] = [
  { device: 'MacBook Pro · Chrome 128', meta: 'Amman, Jordan · 94.187.x.x · active now', icon: 'laptop', current: true, revocable: false },
  { device: 'iPhone 15 · Safari', meta: 'Amman, Jordan · last active 2h ago', icon: 'device-mobile', current: false, revocable: true },
  { device: 'Windows 11 · Edge', meta: 'Irbid, Jordan · last active 3d ago', icon: 'desktop', current: false, revocable: true },
  { device: 'iPad · Safari', meta: 'Aqaba, Jordan · last active 11d ago', icon: 'device-tablet', current: false, revocable: true },
];

export const LOGIN_HISTORY: readonly { who: string; meta: string; result: string; tone: Tone }[] = [
  { who: 'Rania Haddad · Super Admin', meta: 'Today 08:52 · Amman · 2FA', result: 'Success', tone: 'ok' },
  { who: 'Omar Deeb · Support Admin', meta: 'Today 02:14 · Bucharest · 4 attempts', result: 'Blocked', tone: 'bad' },
  { who: 'Yousef Barakat · Finance Admin', meta: 'Yesterday 17:40 · Amman · 2FA', result: 'Success', tone: 'ok' },
  { who: 'Nadia Sweiss · Support Admin', meta: '01 Sep 09:03 · Irbid · 2FA', result: 'Success', tone: 'ok' },
  { who: 'Khalid Amr · Finance Admin', meta: '31 Aug 22:10 · Amman · no 2FA enrolled', result: 'Warning', tone: 'warn' },
  { who: 'Dana Qasem · disabled account', meta: '28 Aug 11:02 · Amman', result: 'Denied', tone: 'bad' },
];
