...
- Penalties are ASSESSED, never charged. Spec 3.3: with no dispute ticket, no penalty is applied at all. Money moves only through an Admin resolving a ticket — with ONE exception, decided by the owner on 2026-09-24: a CUSTOMER who has PAID the deposit and cancels inside the free-cancellation window gets the whole deposit refunded automatically to the original payment method. The refund is recorded in the same save as the cancellation (`DepositRefundSettlement`, reason `FreeCancellation`) and sent by the payment sweep; a free-cancelled booking then holds nothing a dispute could split (`Booking.ReturnsDepositOnCancellation`, `DepositHeldFor`). A gallery or admin cancelling in that window, and every cancellation outside it, keep the existing policy. Do not widen or remove this without the owner.
