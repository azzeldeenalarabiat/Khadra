...
- Real email: `Email:Provider` selects the transport — `Resend` (HTTPS API, the default in Development), `Smtp` (Gmail, Brevo, any relay) or `Logging` (writes to the log, delivers nothing). Credentials are secrets and live ONLY in user-secrets or the environment; tracked appsettings hold placeholders.
...
- Business numbers (commission %, deposit %, no-show hours, penalties, cancellation window, SLA) come ONLY from `IBusinessRulesProvider` (configuration section `BusinessRules` today, admin-editable aggregate later). Never a constant.
